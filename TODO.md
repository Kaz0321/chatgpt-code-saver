...
| 8 | done | Settings | 設定項目の整理を進める | 2026-03-03: lightweight mode の CSS 適用範囲を修正し、`scroll-behavior` 無効化が実際に効くよう更新。`normal / auto / light` と生成中追従の unit test を追加。`Display` に `Lightweight Mode` と `Preview lines` をまとめ、`Display Actions` にコードブロックと heading の一括操作を分離した。`viewSettings` unit test も追加し、WSL の `node --test tests/unit/*.test.js` で 39 件 PASS。 |
