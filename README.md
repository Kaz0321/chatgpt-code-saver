...
- 既定テンプレート文言は `content/defaultTemplate.js` の `DEFAULT_TEMPLATE_CONTENT` で定義されています。`cgptSetTemplates`/`cgptSetSelectedTemplateId` アクセサを経由することで状態破壊的な変更を避け、単一責務を保ちながら拡張できます。
